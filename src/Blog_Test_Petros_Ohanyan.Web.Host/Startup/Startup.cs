    ...
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddSwaggerGen(options =>
            {
                ...
                {
                    Version = _apiVersion,
                    Title = "Blog_Test_Petros_Ohanyan API",
                    Description = "Blog_Test_Petros_Ohanyan",
                    // uncomment if needed TermsOfService = new Uri("https://example.com/terms"),
                    Contact = new OpenApiContact
                    {
                        Name = "Blog_Test_Petros_Ohanyan",
                        Email = string.Empty,
            ...
            return services.AddAbp<Blog_Test_Petros_OhanyanWebHostModule>(
...
app.Use(async (context, next) =>                {                    await next();                    if (context.Response.StatusCode == 404                        && !Path.HasExtension(context.Request.Path.Value)                        && !context.Request.Path.Value.StartsWith("/api/services", StringComparison.InvariantCultureIgnoreCase))                    {                        context.Request.Path = "/index.html";                        await next();                    }                });
            ...
            app.UseSwaggerUI(options =>
            {
                ...
                options.SwaggerEndpoint($"/swagger/{_apiVersion}/swagger.json", $"Blog_Test_Petros_Ohanyan API {_apiVersion}");
                options.IndexStream = () => Assembly.GetExecutingAssembly()
                    .GetManifestResourceStream("Blog_Test_Petros_Ohanyan.Web.Host.wwwroot.swagger.ui.index.html");
